...
def connect_with_middleware(contract_json):
	...
	url = "https://bsc-testnet.infura.io/v3/2ea6510a3e0e4513b3d951b705f321a9"
	w3 = Web3(HTTPProvider(url))
	assert w3.is_connected(), f"Failed to connect to provider at {url}"
...
def is_ordered_block(w3, block_num):
	...
	legacy_transaction = getattr(block, 'baseFeePerGas', False)

	#Legacy Transaction
	if legacy_transaction == False:
		for i in range(len(transactions)-1):
			## Is a gasprice higher than thee proceeding transaction other for every transaction
			if transactions[i]['gasPrice'] < transactions[i+1]['gasPrice']:
				return ordered
	...
	#Type 2 Transaction
		...
		for i in range(len(transactions)):
			if transactions[i].type == 0:
				price.append(transactions[i].gasPrice - block.baseFeePerGas)
			elif transactions[i].type == 2:
				price.append(min( transactions[i]['maxPriorityFeePerGas'], transactions[i]['maxFeePerGas'] - block.baseFeePerGas ))
